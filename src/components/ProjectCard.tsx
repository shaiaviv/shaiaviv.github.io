...
export default function ProjectCard({ project, index }: Props) {
  ...
  const spotlightBg = useMotionTemplate`radial-gradient(220px circle at ${spotX}% ${spotY}%, rgba(0,229,160,0.1), transparent 70%)`
  ...
  return (
    <motion.div
      ...
      className="glass-card rounded-2xl p-6 flex flex-col relative overflow-hidden group h-full
        hover:border-accent/15 hover:shadow-[0_12px_40px_rgba(0,229,160,0.08),0_0_0_1px_rgba(0,229,160,0.06)]
    ...
    >
      ...
      <div className="flex items-start justify-between mb-5 relative z-10">
        <motion.div
          ...
          className="p-2.5 rounded-xl bg-accent/8 border border-accent/15"
        >
        ...
        <div className="flex gap-2">
          {project.repo && (
            <a href={project.repo} target="_blank" rel="noopener noreferrer"
              className="w-9 h-9 rounded-full flex items-center justify-center bg-white/[0.04] border border-white/[0.08] text-muted hover:text-text-1 hover:border-accent/30 hover:bg-accent/8 transition-all duration-200" aria-label="GitHub">
          ...
          {project.live && (
            <a href={project.live} target="_blank" rel="noopener noreferrer"
                            className="text-muted hover:text-text-1 transition-colors duration-200" aria-label="Live demo">
      ...
      <div className="flex items-center gap-3 mt-auto flex-wrap relative z-10">
        ...
        <div className="flex gap-1.5 flex-wrap">
          {project.tags.slice(0, 3).map((tag) => (
            <span key={tag}
              className="font-mono text-xs text-accent/70 bg-accent/8 border border-accent/10 px-2 py-0.5 rounded-md">
              {tag}
            </span>
          ))}
        </div>
